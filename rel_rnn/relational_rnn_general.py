...
class RelationalMemory(nn.Module):
    ...
    def attend_over_memory(self, memory):
        ...
        """
        ### memory: [batch_size, mem_slot+1, self.mem_size]  
        for _ in range(self.num_blocks):
            
            ### memory: [batch_size, mem_slot+1, self.mem_size]  
            ### attended_memory: [batch_size, mem_slot+1, self.mem_size]  
            ...
            ### attention_mlp: [batch_size, mem_slot+1, self.mem_size]  
    ...
    def forward_step(self, inputs, memory, treat_input_as_matrix=False):
        ...
        if treat_input_as_matrix:
            ...
            ### inputs: [batch_size, input_size] --> inputs: [batch_size, self.mem_size]
            inputs = self.input_projector(inputs)
            
            # unsqueeze the time step to dim 1
            ### inputs_reshape: [batch_size, 1, self.mem_size]
            inputs_reshape = inputs.unsqueeze(dim=1)

        ### memory: [batch_size, mem_slots, self.mem_size]
        ### inputs_reshape: [batch_size, 1, self.mem_size]
        ### memory_plus_input: [batch_size, mem_slot+1, self.mem_size] 
        memory_plus_input = torch.cat([memory, inputs_reshape], dim=1)
        
        ### memory_plus_input: [batch_size, mem_slot+1, self.mem_size] --> next_memory: [batch_size, mem_slot+1, self.mem_size]
